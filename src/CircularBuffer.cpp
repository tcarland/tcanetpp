...
CircularBuffer::read ( void *buff, size_t n )
{
    ...
    while ( rd > 0 && ptr != nullptr )
    {
        if ( (rptr = getReadPtr(&size)) == nullptr )
            break;
...
CircularBuffer::write ( const void *buff, size_t n )
{
    ...
    while ( wd > 0 && ptr != nullptr )
    {
        size = wd;

        if ( (wptr = getWritePtr(&size)) == nullptr )
            break;
...
CircularBuffer::reverse ( size_t offset )
{
    ...
    if ( offset > rpos ) {
        offset = 0;
    } else if ( _endPtr == nullptr && offset > (size_t)(_readPtr - _buffer) ) {
...
// ----------------------------------------------------------------------
    ...
    /**   Returns a char* pointing to the first available write location
      ...
      *   will return nullptr.  The size parameter is populated with the
      ...
      *
      *   A 'nullptr' is returned on error.
     **/
...
CircularBuffer::getWritePtr ( size_t * size )
{
    ...
    if ( _endPtr == nullptr && offset < *size ) {
...
/**   Accepts @param  offset as the size written to the buffer,
  ...
  *
  *   The method will false if the offset provided is not valid.
 **/
bool
CircularBuffer::setWritePtr ( size_t offset )
{
    if ( offset > this->writePtrAvailable() )
        return false; //CircularBuffer over-write
...
/**   Returns the readPtr to the first readable position if
  *   there is space to read, or returns nullptr on no data.
  *
...
CircularBuffer::getReadPtr ( size_t * size )
{
    ...
    *size = rd;
    ...
    if ( rd == 0 && _endPtr != nullptr ) {   // read wrap
        _readPtr = _buffer;
        _wrapPtr = _endPtr;
        _endPtr  = nullptr;
...
/**   This method sets the read pointer to the new position or
  *   returns false if the offset provided would cause an overread.
  *
...
CircularBuffer::setReadPtr ( size_t offset )
{
    if ( offset > this->readPtrAvailable() )
        return false; //CircularBuffer over-read

    _readPtr += offset;

    if ( _endPtr != nullptr && _readPtr == _endPtr ) {   // read wrap
        _readPtr = _buffer;
        _wrapPtr = _endPtr;
        _endPtr  = nullptr;
    }
